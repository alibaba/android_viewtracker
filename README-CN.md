...
`ViewTracker`是用于自动化的采集用户UI交互过程中的点击和曝光事件，基于`view`事件代理及过滤的数据采集库。

### 0.1 功能特性

* 支持`Android` & `iOS`（[iOS github repo](https://github.com/alibaba/TMViewTrackerSDK)）平台。
* 支持点击、曝光、页面公共信息。
...
* `OCP`（开闭原则）：最大化支持用户扩展开发，如数据提交`DataCommitImpl`接口定义，动态配置监控的`receiver`提供。
